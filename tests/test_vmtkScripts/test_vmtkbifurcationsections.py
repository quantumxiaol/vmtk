...
def test_bifurcation_section_group_ids_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_bifurcation_group_ids_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_point_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_normal_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_area_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_min_size_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_max_size_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_shape_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_closed_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_orientation_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
...
def test_bifurcation_section_distance_to_spheres_correct(bifurcation_sections, expectedValue, request):
    bifurcation_sections = request.getfixturevalue(bifurcation_sections)
    wrapped_bifur_section = dsa.WrapDataObject(bifurcation_sections)
